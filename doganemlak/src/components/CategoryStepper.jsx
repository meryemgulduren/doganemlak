          ...
          <div key={i} className="flex items-center">
            <div className="flex flex-col items-center">
              <motion.div
                animate={
                  active
                    ? { scale: 1.15, boxShadow: "0 0 0 4px rgba(44,78,207,0.18)" }
            ...
            {i < steps.length - 1 && (
              <div
                ...
                style={{ background: done ? "#2F855A" : "#e2e8f0" }}
              />
            )}
          </div>
        );
          ...
          <AnimatePresence mode="wait" custom={direction}>
            ...
            {!selection.category && (
              <motion.div key="step-main" custom={direction} variants={slideVariants}
                ...
                <p className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-5">Ana Kategori Seçin</p>
            ...
            {selection.category && !selection.listingType && (
              <motion.div key="step-type" custom={direction} variants={slideVariants}
                initial="enter" animate="center" exit="exit">
                <p className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-5">İlan Tipini Seçin</p>
                <div className="grid grid-cols-2 gap-5 max-w-lg">
                  {listingTypes.map((t) => (
                    ...
                    >
                      {t.id === "SATILIK" ? (
                        <svg className="w-14 h-14 text-slate-400 group-hover:text-primary" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 48 48" strokeLinecap="round" strokeLinejoin="round">
                          <path d="M24 6v36M12 18l12-12 12 12" />
                          <path d="M10 42h28" />
                          <circle cx="24" cy="30" r="4" />
                        </svg>
                      ) : (
                        <svg className="w-14 h-14 text-slate-400 group-hover:text-warning" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 48 48" strokeLinecap="round" strokeLinejoin="round">
            ...
            {selection.listingType && hasSubTypeStep && !selection.subType && (
              <motion.div key="step-sub" custom={direction} variants={slideVariants}
                initial="enter" animate="center" exit="exit">
                <p className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-5">Konut Tipini Seçin</p>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  {currentSubTypes.map((s) => (
                    ...
                    >
                      <div className="w-2.5 h-2.5 rounded-full bg-slate-300 group-hover:bg-primary transition-colors flex-shrink-0" />
              ...
              <motion.div key="step-done" variants={bounceVariants} initial="hidden" animate="visible"
                ...
                <div className="flex gap-3">
                  <button type="button"
                    ...
                    className="px-5 py-2.5 rounded-xl border-2 border-border text-muted font-medium text-sm hover:border-slate-300 hover:bg-slate-50 transition-colors">
                    Değiştir
                  </button>
                  <motion.button
                    type="button"
